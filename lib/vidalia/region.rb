module Vidalia
  ...
  class Region < VidaliaArtifact
    ...
    def initialize(opts = {})
      ...
      if @aliases
        unless @aliases.is_a?(Array)
          raise "Vidalia::Region requires aliases to be an array"
        end
        @aliases.each do |my_alias|
          raise "Vidalia::Region requires each alias in the array to be a string" unless my_alias.is_a?(String)
        end
      end

      if @parent
        if @parent.is_a? Vidalia::Page
          add_to_page(@parent)
        else
          if @parent.is_a? Vidalia::Region
    ...
    # *Options*
    #
    # +page+:: specifies a Vidalia::Page object to add this Region to
    #
    # *Example*
    #
    #   mypage = Vidalia::Page.new(
    ...
    # *Options*
    #
    # +region+:: specifies a Vidalia::Region object to add this Region to
    #
    # *Example*
    #
    #   existing_region = Vidalia::Region.new(
    #     :name => "User"
    #   )
    #   new_region = Vidalia::Region.new(
    #     :name => "Roles"
    #   )
    ...
    def add_to_region(region)
      ...
      if region
        case
        when region.is_a?(Vidalia::Region)
    ...
    #
    # The defined filter should alter the +Thread.current[:vidalia_region]+
    # value.
    #
    # *Options*
    #
    # +block+:: specifies the block of code that will reduce the scope of HTML by updating +Thread.current[:vidalia_region]+.  The block should take a single parameter indicating data unique to the filtered region.  For example, in a list of users, the block parameter could be a user ID.
    # 
    # *Example*
    #
    #   # Define the application
    ...
    #     Thread.current[:vidalia_region] =  Thread.current[:vidalia_region].div(:id => "userlist").div(:text => /#{userid}/)
    #   }
    ...
    # *Example*
    #
    #   region1 = Vidalia::Region.new(
    #     :name => "User List"
    #   )
    #   page.add_region(region1)
    #   region2 = Vidalia::Region.new(
    #     :name => "User Roles"
    #   )
    ...
    def add_region(region)
      ...
      if region
        if region.is_a?(Vidalia::Region)
          ...
          raise "Region must be a Vidalia::Region when being added to another Region"
        end
      else
    ...
    def add_control(control)
      ...
      if control
        if control.is_a?(Vidalia::Control)
          ...
          raise "Control must be a Vidalia::Control when being adding to a Region"
        end
      else
